...
PaymentStrategy is an interface that defines the strategy contract. It declares
the pay method that concrete strategies must implement.

CreditCardPaymentStrategy and PayPalPaymentStrategy are concrete strategies
that implement the PaymentStrategy interface. They provide their own
implementations of the pay method specific to credit card and PayPal payments,
respectively.

PaymentContext is the context class that utilizes the selected payment strategy
to process a payment. It takes a payment strategy object as a parameter in its
constructor and provides the process_payment method to initiate the payment
process.

In the usage example, we create instances of the concrete payment strategies:
CreditCardPaymentStrategy and PayPalPaymentStrategy. Each strategy has its own
set of parameters representing the necessary information for that payment
...
credit card and paying with PayPal. We create the PaymentContext instance with
the corresponding payment strategy and call the process_payment method with the
...
class PayPalPaymentStrategy(PaymentStrategy):
    ...
    def pay(self, amount):
        print(f"Paid ${amount} with PayPal account card {self.email}")
...
credit_card_strategy = CreditCardPaymentStrategy(
    "1234567890", 'John Doe', '12/2023', '123')
paypal_strategy = PayPalPaymentStrategy('junk@junk.com', 'password')

payment_context = PaymentContext(credit_card_strategy)
payment_context.process_payment('1000')

payment_context = PaymentContext(paypal_strategy)
payment_context.process_payment('500')
